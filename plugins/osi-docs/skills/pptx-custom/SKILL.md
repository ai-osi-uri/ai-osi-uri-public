...
## 着手前チェック（ここだけは、書き始める前に必ず読む）

本文は長い。**全部を読んでから書き始めることは実際には起きない**ので、事故に直結する項目だけを先頭に置く。
以下は本文の要約であり、詳細は各節にある。

1. **色は相手から取り直す。** 前のデッキの定数をコピーして書き始めない（→ 色の節）
2. **箇条書きは N（1行に入る字数）を計算してから書く。** N をわずかに超える長さが最悪（→ CP6）
3. **枠の高さは中身から逆算する。** 文字を入れてから枠を詰めない（→ CP7）
4. **手動改行（`\n` ＋ 全角スペース）とぶら下げインデントを併用しない。** 頭が揃わなくなる（→ CP6）
5. **アルファを付けた後に減色しない。** 円形マスクが外れる（→ 画像の節）
6. **描画したら、まず枠外・重なりを機械で検出する**（→ QA の「はみ出しの機械チェック」）
7. **全ページを画像にして目で見るまで success と言わない**（→ Visual QA）
8. **中身が2〜3行しかない項目を、縦長カードの横並びにしない。** カードの高さを紙の下端まで引き伸ばすと下半分が空白になる。中身が少ない一覧は「上から下のリスト」で刷る（→ CP8）

> このリストは、実際に事故った項目だけで作っている。増やすときは「実際に起きたか」で判断する。
...
### 寸法の基準定数（全節でこれを使う。各節にベタ書きしない）

寸法の見積もりは本ファイルの複数の節で出てくる。**係数は必ずここから引く**（節ごとに違う数字を書くと、同じスライドに二つの答えが出る）。

| 量 | 式 | 10.5pt の例 |
|---|---|---|
| 日本語1文字の幅 | `pt ÷ 72 ÷ 0.92` インチ | 0.159" |
| 1行に入る全角字数 N | `テキスト幅inch × 72 ÷ pt × 0.92` | 2.5"幅 → 約16字 |
| 1行の高さ（行間100%） | `pt × 0.0176` インチ | 0.185" |
| セル内の上下余白 | 約 0.12" | — |

英字は全角の約半分（`pt ÷ 72 ÷ 1.85`）で見る。
...
**2. テキストがコンテナを溢れる。** pptxgenjs も python-pptx も自動縮小しない。可変長テキスト（価格・要約・説明）は、ほぼ全幅（10"スライドで8–9"）のボックスにするか、文字数から必要幅を計算する（幅は「寸法の基準定数」から引く）。
...
**孤立を実際に消すのは文字数の調整だけ。** ぶら下げインデントは見た目を整えるが、孤立行そのものは1文字も減らない。
両方やるが、役割を混同しない。

**(a) 文字数を N に合わせる（これが本命）**
...
折り返し行が行頭「・」「①」の下に潜り込むのを防ぐ。python-pptx に API が無いので `pPr` を直接触る。

```python
from pptx.util import Emu, Inches

def hang(p, pt=10.5):
    """折り返し行を1文字分ぶら下げる。ぶら下げ幅はフォントサイズから決める"""
    inch = pt / 72 / 0.92                      # 全角1文字の幅
    pPr = p._p.get_or_add_pPr()
    pPr.set("marL", str(int(Emu(Inches(inch)))))
    pPr.set("indent", str(-int(Emu(Inches(inch)))))
```

**(b) を入れると2行目の字数が減る。** marL の分だけ2行目は狭くなるので、
2行に収まる上限は `2N` ではなく **`N + (N − 1)` ≒ 2N−1 字**（上例なら約30字）。(a) の見積もりに反映する。

同じ理屈で、**カード内の見出し＋説明は1セット2行に収める**設計が安全。狭い列に3行以上を入れない。

**7. 文字が枠線・罫線に接触する。**
枠と文字の間に余白が無いと、下端の数ピクセルが「切れている」ように見える。
python-pptx / pptxgenjs はテキストフレームの内側余白の既定が小さく、
**枠の高さを文字量にぴったり合わせると必ず起きる**（特に、後から1行足したとき）。

一般則：**文字と枠線の間に、フォント高の1/3以上の余白を必ず取る**（10ptなら約0.05"）。

- 枠の高さは「行数 × 行高 + 上下余白 × 2」から決める。文字を入れてから枠を詰めない
- テキストフレームの内側余白を明示する（`tf.margin_top` 等。`margin:0` にするのは、
  罫線を文字端に揃えたい場合だけで、そのときは枠線を引かない）
- 可変長テキストを入れる枠は、**想定より1行分高くしておく**（最も安いバグ予防）

**8. 中身の少ないカードを、紙の下端まで引き伸ばして横に並べる（「作りかけ」に見える）。**
...
- 一覧（3〜6項目、各項目が見出し＋1〜2文）は、**上から下のリスト**で刷る：左に番号＋見出し（固定幅）、右に説明、
  行の高さは中身から決め、紙に収まらないときだけ全体を縮める。空白は出ない
- 横並びカードを使うのは、各カードに画像・図・3行以上の本文があり、枠を埋められるときだけ
- カードの高さを「使用可能高 ÷ 行数」で一律に決めるヘルパーは、**中身から高さを決めるヘルパーに差し替える**
  （高さ＝見出し行 ＋ 本文の推定行数 × 行高 ＋ 上下余白。列内で最大の高さに揃える）
- 目次・「やること N 個」・「決めてもらうこと N 個」のような項目列挙は、ほぼ常にリストが正しい
...
**型D：試作案カード（3〜5件の一覧）** — 列＝試作案ごとに1列、各列の縦＝`現場で起きていそうなこと / 試作案 / 効き目`
- 列見出しは `NN ＋ 動詞で終わる名前`。3段のラベルは色で役割を分ける（現状＝ダーク／提案＝強調色／効き目＝成功色）。
- 1セル60字以内。3列で入らないなら件数を減らす（4件以上は2枚に割る）。
- 「現場で起きていそうなこと」には（推測）と分かる語を残す。断定に見えると評価になる。

**型E：実現策（1試作案1枚）** — 左上`現場で起きていそうなこと`／左下`試作案（何を作るか）`／右`AIを入れた後（1日の流れ）`／下3カード`AIが出すものの例 / 効き目と測り方 / 最初の目標`
- 1日の流れは `時刻 / 担当（AI or 人） / やること` の行を3〜5本。**AIと人の担当を色で分け、人の行には所要時間を書く**（「人は承認だけ」が一目で分かる）。
- 「AIが出すものの例」は**画面やメールに実際に出る文面**を固有名詞入りで1つ。抽象語（「最適な提案」）は不可。
- 5行×0.45"＋見出しで右パネルは約2.75"要る。左の2ボックスの合計高さを右に揃える（実測：2026-09、8〜9pt）。
...
- それでも大きいなら減色する。**ただしアルファを付けた後に減色しない。**
  `quantize().convert("RGB")` は**アルファチャンネルを捨てるので、円形マスクが外れて四角い写真に戻る**。
  減色するなら crop 直後（`putalpha` の前）に置く：
  ```python
  im = im.crop(...).resize((px, px), Image.LANCZOS)
  im = im.quantize(colors=192).convert("RGB")   # ← putalpha より前
  im.putalpha(mask); im.save(out, optimize=True)
  ```
- それでも通らないときは**画質を落として通すより、経路を変える**（ローカルのDriveミラーに `cp`。`proposal-package` 参照）。
  **納品物の品質を、運搬経路の都合で下げない。** これは pptx に限らず、サイズ上限のあるコネクタ全般に当てはまる。
...
2. **コーポレートサイトのロゴ画像を canvas で集計**（IR PDF が手に入らないときの本命。数十秒で終わる）
   サイトを開き、ロゴ画像を canvas に描いてピクセルを数える。
   **マークが多色の会社では色数そのものが分かる**（＝相手が何色で運用しているかが取れる）。
   ...
   await (async () => {
     ...
     for (let i = 0; i < d.length; i += 4) {
       ...
       if (Math.abs(r-g) < 15 && Math.abs(g-b) < 15) continue;   // グレー・白黒を除外
       const k = "#" + [r,g,b].map(v => (v>>3<<3).toString(16).padStart(2,"0")).join("");
       cnt[k] = (cnt[k]||0) + 1;
     }
     const total = Object.values(cnt).reduce((a,b)=>a+b,0);
     return Object.entries(cnt).filter(([,n]) => n/total > 0.01)   // 1%未満はアンチエイリアスの中間色
                  .sort((a,b)=>b[1]-a[1]).slice(0,8);
   })()
   ```
   **グレー除外と1%足切りを省かない。** ロゴPNGの縁はアンチエイリアスの中間色だらけで、
   省くと上位が「ブランド色と白の中間のくすんだ色」で埋まる（IR PDF 版と同じ理由）。

   **ロゴ候補は複数試す。** ヘッダー用（白抜き）は白1色しか返らないことがあり、マーク版に本来の色が入っている。
   パスは DOM から拾う：`[...document.images].map(i=>i.src).filter(s=>/logo|brand|mark/i.test(s))`

   **canvas が使えない場合がある。** 画像サーバが CORS ヘッダを返さないと `crossOrigin` 付き読み込みは失敗し、
   外すと canvas が汚染されて `getImageData` が SecurityError になる。**そのときは 3 に落とす。**
3. **コーポレートサイトの CSS を集計**（canvas が使えないとき／サイト運用色を知りたいとき）
   ブラウザツールで `getComputedStyle` を集計する。ロゴ色とは別に、
   **相手が実際に運用しているアクセント色**（CTAボタン・見出し帯・リンク）が取れる。
   ロゴが1色でもサイトが別のアクセント色で回っている会社はあるので、**2 が成功しても一度は見る**。
4. ロゴ画像を手元に落としてピクセルサンプリング（1〜3 がいずれも使えないとき）

**取れた色を、相手の使い方どおりに使う。** ブランド色が1色しかない会社に無理やり4色の意味割り当てをしない。
「メイン1色＋サブ1色＋グレー」で運用している相手なら、こちらも3色で組む。**色数は相手のIR資料に合わせる。**

**1色しか無い相手には、色を足さずに濃度を作る。**
取れた1色をそのまま見出しにも帯にも使うと、淡ければ読めず、濃ければ重い。
同じ色相のまま **濃（文字・見出し）／中（帯・線）／極淡（面）** の3段を作れば、1色運用のまま情報の階層が付く。
役割色（注意して扱う論点など）が要るときも、**足すのは1色まで**。

> 実例（2026-09）：淡い青緑1色のサイトから `#9DD0CE` を取得。文字に使えないので
> 濃 `#2C6E6B` ／ 中 `#9DD0CE` ／ 極淡 `#E7F3F2` の3段を作って組んだ。
...
### はみ出しの機械チェック（画像化より先に、これを通す）

**原則：シェイプの座標は、目で見る前にコードで検算する。**
枠外にはみ出した要素とページ番号との重なりは、**計算で確実に出る**のに、目視QAに回すと見落とす。
レンダリング前に潰せるので、画像化より先に実行する。

```python
from pptx import Presentation
from pptx.util import Emu

SW, SH, M = 10.0, 5.625, 0.55        # スライド寸法と外余白
...
for i, s in enumerate(prs.slides, 1):
    for sh in s.shapes:
        ...
        t = (sh.text_frame.text[:24].replace("\n", " ") if sh.has_text_frame else sh.shape_type)
        if B > SH - 0.30 or R > SW - 0.30 or L < -0.05 or T < -0.05:
            print(f"[枠外] p{i} B={B:.2f} R={R:.2f} :: {t}")
```

**判定の目安：** 下端が `SH − 0.30"`（＝ページ番号の帯）を超えたら赤。背景の敷き板は除外してよい。
0件になるまで座標を直してから画像化する。

> なぜ効くか：**表と縦積みは、行を1つ足しただけで下端が数値的に破綻する。**
> 「BY + 各行の高さ」を毎回手で足し直すのは現実的でないので、最後に一括で検算する。
...
**サブエージェントを使う**（2–3枚でも）。自分はコードを見続けて「期待」を見てしまう。サブエージェントは新鮮な目。
**サブエージェント禁止の環境では、自分で全ページを画像化して1枚ずつ見る。省略しない。**

スライドを画像化（下記）し、次の観点で点検する。**各スライドごとに、軽微でも全て報告させる。**

**はみ出し・重なり**
- 要素同士の重なり／テキストの溢れ・切れ
- **表の最終行が下のカードに隠れていないか**（CP5）
- **文字が枠線・罫線に接触していないか**（CP7。下端の数pxは「切れている」ように見える）
- バッジ／ラベルがタイトル文字に重なっていないか（CP1）

**折り返し**
- **末尾の数文字だけが次行に落ちていないか**（CP6）
- 1行用の装飾線を敷いたところでタイトルが2行化していないか
- 閉じ括弧や助詞1文字だけの行ができていないか

**余白・整列**
- 端からの余白不足（<0.5"）／要素が近すぎる（<0.3"）
- 不揃いな余白／列や行の高さの不揃い／脚注の衝突
- **カードの下半分が空白になっていないか**（CP8。中身が少ない横並びカードは縦のリストに直す）

**可読性**
- 低コントラストの文字・アイコン／狭すぎるテキストボックス

**素材**
- 画像の歪み／AI画像の文字アーティファクト／プレースホルダ残り

> サブエージェントに渡すときは「**軽微でも全て報告。問題なしと書くな**」と明示する。曖昧に頼むと「概ね良好です」で返ってきて、
> 孤立行や枠線接触のような**単体では軽微だが枚数分累積して"雑に見える"欠陥**が素通りする。

**依頼文には「見た目」だけでなく「内容・表現」の観点も必ず入れる。**
見た目だけ頼むと、**提案の主張を自分で無効化している一文**のような致命的な欠陥が素通りする。
最低限、次の3行を依頼文に足す（顧客・案件の性格に合わせて書き換える）：

```
- この資料は「◯◯を相手に決めてもらう」ためのものです。要求が伝わらない箇所、
  逆に相手を責めるように読める箇所はありませんか
- 断定しすぎ・約束しすぎに読める箇所はありませんか（特に責任・費用・期間）
- 資料内の数字（◯名、◯社、◯つ）が互いに矛盾していませんか
```

> 実例（2026-09）：見た目のみで依頼した回は溢れしか出なかった。観点を足した回に、
> 「提案の対象を自ら例外化していた一文」と「5つ決めたいと言って3つしか聞いていない不整合」が出た。
