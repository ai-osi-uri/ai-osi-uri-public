...
議論用資料で slide-plan が「目次を章ごとに再掲」と指定しているときは、再掲の目次が扉を兼ねる（次項）。この場合の一致確認は「章の数 ＝ 再掲目次の枚数」。
...
### 目次・章ラベル・「決めていただきたいこと」は、一つのリストから機械生成する

**目次の小項目名・各スライド上部の章ラベル・最終ページの論点表を、別々に手書きしない。** slide-plan の一覧を1つの配列（AGENDA）に持ち、目次スライドも章ラベルもそこから作る。手書きすると要約の仕方が場所ごとに変わり、「目次とタイトルが合っていない」になる（→ deck-composition 原則7）。

**議論用資料の目次は、章の区切りごとに再掲し、これから話す章だけ本文色・他の章はグレーにする**（slide-plan に「章ごとに再掲」の指定があるとき。指定が無ければ従来どおり先頭1枚＋章扉）。再掲の目次は章扉を兼ねてよい。

```python
AGENDA=[  # (章番号, 章名, [(小項目番号, 小項目名=スライドタイトルと同一), ...], この章で話すこと)
 ("1","全体の見通し（結論）",[],"結論を先に5行で"),
 ("2","資料から読み取った現在の状態",[("2.1","機能と実装状況（事実）"),("2.2","決まっていること・未決のこと（事実）")],"…"),
]
def lab(sec,i=None):                     # スライド上部の章ラベル。タイトルはこの小項目名をそのまま使う
    num,name,subs,_=AGENDA[sec]
    return f"{num}. {name}" if i is None else f"{num}. {name}｜{subs[i][0]} {subs[i][1]}"
def toc(active):                         # 章の区切りに挿入。active の章だけ INK（本文色）・太字、他は GRAY
    for k,(num,name,subs,_) in enumerate(AGENDA):
        color = INK if k==active else GRAY
        ...                              # 2列に分けて縦に並べる（項目が多いと1列では紙からはみ出す）
...
- 目次の小項目名とスライドタイトルを突合する（AGENDA から生成していれば一致する。手書き部分が残っていないか grep）
