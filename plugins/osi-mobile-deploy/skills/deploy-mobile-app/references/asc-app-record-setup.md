...
**原因**：Prod flavor 用の Bundle ID `{{company.reverse_domain}}.sampleapp.prod` が
Apple Developer Portal に登録されていなかった（Dev flavor `{{company.reverse_domain}}.sampleapp.dev`
...
→ Description: `SampleApp Prod`（人が読める名前、後で変えられる）
→ Bundle ID: Explicit → `{{company.reverse_domain}}.sampleapp.prod`
...
bundle exec ruby -r spaceship -e ''
  ...
  b = Spaceship::ConnectAPI::BundleId.all(filter: { identifier: "{{company.reverse_domain}}.sampleapp.prod" }).first
...
→ Platforms: iOS
→ Name: `SampleApp`（ユーザ表示名。既存アプリと衝突しないこと）
→ Primary Language: 日本語 (Japanese)
→ Bundle ID: `{{company.reverse_domain}}.sampleapp.prod`（step 1 で登録した ID をプルダウンから選ぶ）
→ SKU: 案件横断でユニークな任意文字列（例: `sampleapp-prod-2026`）
...
bundle exec ruby -r spaceship -e ''
  Spaceship::ConnectAPI.token = Spaceship::ConnectAPI::Token.create(
    key_id: ENV["ASC_KEY_ID"], issuer_id: ENV["ASC_ISSUER_ID"],
    key: Base64.strict_decode64(ENV["ASC_P8_B64"]))
  app = Spaceship::ConnectAPI::App.find("{{company.reverse_domain}}.sampleapp.prod")
...
export TARGET_BUNDLE_ID="{{company.reverse_domain}}.sampleapp.prod"
export TARGET_BUNDLE_NAME="SampleApp Prod"
...
```
✅ Bundle ID {{company.reverse_domain}}.sampleapp.prod (id=VC537CXA23) registered/exists
⚠️  App record on ASC: MISSING
    → Web UI に移動して手動作成が必要:
       https://appstoreconnect.apple.com/apps/new/app
       Name: SampleApp, Bundle ID: {{company.reverse_domain}}.sampleapp.prod, SKU: sampleapp-prod-2026
```
...
```
✅ Bundle ID {{company.reverse_domain}}.sampleapp.prod (id=VC537CXA23) registered/exists
✅ App record on ASC: 6801234567 — SampleApp
...
SampleApp は dev のみ動かしていたが、prod TestFlight を配りたくなった瞬間に本問題を踏む。
...
Bundle ID に紐づく。`com.example.myapp` を絶対に本番で使わない、`{{company.reverse_domain}}.*`
...
App Store で完全に独立して並行運用できる。ASC 上の App 名は `SampleApp` / `SampleApp Dev` のように
分けると管理しやすい。
