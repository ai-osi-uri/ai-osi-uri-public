...
| **判定（Jev）** | **要 / 不要**（主要機能に「振り分け・チェック・分類・点数付け」があれば要） |
...
**判定が「要」なら [references/jev-in-app.md](references/jev-in-app.md) を読んでから実装する。**
判定の部分は Claude ではなく Jev で作り、自信の低いものを人が確認する画面を必ず付ける。
お客様に納めるアプリの鍵はお客様自身の TypeSafe の鍵（当社の鍵をアプリに入れない）。
