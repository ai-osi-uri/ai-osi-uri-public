...
拡張の設定欄に以下を入力（必要なものだけでよい）。**発行は人に探させず、`credential-handoff` で
発行画面の直前まで Claude が連れて行き、3行（何の鍵か／ここからコピー／ここに貼る）で渡す。**
下の表は「どこで生まれるか」の当たりであって手順ではない（画面は変わるので、その場で読んで進む）。発行先：
...
> (`.p12`) も同時に基本検証する。詳細は `ios-appstore-release/references/n1-policy.md`（osi-mobile-deploy）。
