...
name: app-smoke-test
description: |
  デプロイ済みの URL を curl で叩いて HTTP レベルの動作確認をする。
  パス・メソッド・期待ステータス・本文一致を指定でき、結果をサマリ JSON で返す。
  `create-app` / `update-deploy` から呼ばれる。単体では「スモークテストして」
  「公開した URL を叩いて確認」「ヘルスチェック走らせて」で発動。実カードでの E2E
  決済テストは対象外（人に依頼する）。
version: 0.1.1
---
