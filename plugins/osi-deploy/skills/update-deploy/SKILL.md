...
## Phase 5.5: マニュアルの追随（条件付き・報告前のゲート）
...
- メール文面の追加・削除
- 課金の条件・金額の変更
- **公開 URL が変わった**（別プロジェクトへ移した・独自ドメインを当てた）
- **アプリを分割した／統合した／改名した**（1つが2つになった、名前が変わった）
...
完了報告に**必ず**次の1行を入れる。空欄で報告しない。

```
マニュアル: 変更なし ／ v{n} に更新（{変わった点}）／ 不要（{理由}）
```

**「マニュアルは？」と後から聞かれた時点で、この Phase は失敗している。**
実際に起きた事故（IB CLINIC・2026-09）：1つのアプリを 2 つに分けて公開したのに、
マニュアルは 1 本ものの URL と「患者画面を開く」ボタンの手順のまま残った。
ユーザーに指摘されるまで気づかなかった。**URL が変わったら手順書は必ず嘘になる。**

### この Phase をスキル経由でない作業でも通す

本スキルを使わずに手で直した・別リポを立てて公開した場合も、上のリストに当たるなら
同じことをする。**「update-deploy を使わなかったから Phase 5.5 は関係ない」は通らない。**
判断の基準は使ったスキルではなく、**現物と手順書が食い違ったかどうか**。
...
- **URL・画面構成・ボタン名を変えたのに、マニュアルを直さずに完了報告を出さない**（Phase 5.5）。
  「マニュアルは別途」と言い残して終えない。同じ作業の中で直す。
