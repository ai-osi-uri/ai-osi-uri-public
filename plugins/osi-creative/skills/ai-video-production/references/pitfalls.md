...
## 24. アニメアバター化は「盛られる」（2026-08 発見）

写真からアニメアバターを作ると、モデルは**顔を細く・若く・整った顔立ちに寄せる**。実在人物では別人になり、
自己紹介動画としては致命的。実際に1回目は完全な別人（美男子）が出た。

- プロンプトに **`do not slim` / `do not beautify` / `no sharp chin` / `not a handsome anime lead`** を明示。
- 保ちたい特徴を**名指しで列挙**：丸い輪郭／頬の厚み／目の細さ／眉の太さと位置／年齢感／体格。
- **必ず写真と並べて見る。** 単体だと「良い絵」に見えて、似ていないことに気づけない。
- **本人の承認を取るまで先へ進まない。** 省略しない唯一の工程。

## 25. シーン派生は `edit_image`→`i2v` が reference-to-video より確実で安い（2026-08 発見）

同一人物を複数カットに出すとき、`seedance25-ref` 等より、**承認済みアバターから `edit_image` で各シーンの
キービジュアルを作り、それを `i2v` の起点にする**方が良い。

- `i2v` は**渡した画像がそのまま1フレーム目**になるので、顔のブレが構造的にゼロ。
- 承認を取った構図と顔がそのまま動く（ref はシーンを再生成するので作り直しになる）。
- コストが約1/5（veo31-fast $0.10/s 対 seedance25 720p $0.473/s）。
- ただし **`edit_image` でも顔は盛られる**（#24）。派生ごとに顔を確認し、崩れた1枚だけ作り直す。

## 26. ナレを先に作って実測してからビート尺を決める（2026-08・2回踏んだ）

ElevenLabs v3 は**同じ字数でも尺が読めない**（間の取り方が変わる）。ビート尺を先に決めて原稿を書くと
ほぼ必ず溢れる。実測で8秒想定のビートに 12.2秒 / 7.9秒 のナレが出た。

- **順序を入れ替える**：台本 → **ナレ生成 → 実測** → ビート尺確定 → 絵 → 動画。
- 各ビートは **ナレ実測 + 0.35秒（頭）+ 0.25秒（尻）** を下回らないこと。
- 溢れたら速度を上げる前に**文を削る**。`speed` は 1.05 程度まで。
- 本人の言葉（B5）を削るときは**必ず本人に確認する**。
