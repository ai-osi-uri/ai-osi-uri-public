...
`generate_image`（model: `nano-banana-2`、4K、アスペクトは尺に合わせ 16:9 か 9:16）。
画像プロンプトの型：
...
被写体を差し替えるだけで同質の空撮が作れる。コスト目安：画像 nano-banana-2 約$0.08 ＋ Seedance 2.0 15秒（≒$4.5）。試作は `seedance20-mini-ref` で先に構図を確認すると安い。
