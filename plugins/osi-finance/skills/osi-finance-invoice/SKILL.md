...
- **操作者トラッキング（v3）**：**コネクタが自動で記録するので、`actor` を渡さないこと。** 拡張の設定「操作者」の値を サーバ側（`xlsx-tools`）が 更新者・更新日時（新規行なら 作成者・作成日時）に押す。**ログインユーザーのメールを自分で入れないこと**——経路ごとに違う文字列が入り、台帳の操作者が割れる（実際に `yuhenagisa@ai-osi-uri.com` 743行と `Claude(OSI Finance)` 77行に割れた）。正本: `assets/schema/data-layout.yaml` の `operator_tracking`。列はヘッダ名で解決する。
