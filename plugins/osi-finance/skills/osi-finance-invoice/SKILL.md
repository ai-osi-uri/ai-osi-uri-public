...
description: >
  ...
  （CCあり）宛の Gmail 下書きをPDF添付で作成し、台帳のステータスを「下書き作成済」に更新する。
...
### 6. 台帳更新
- 各行：請求ステータス→**「下書き作成済」**、請求書番号(=MF billing_number)・請求日・支払期限・**送付請求書ファイル（DriveリンクまたはパスとMF billing_id）**を記入。
- **操作者トラッキング（v3）**：更新した各行の **更新者＝Cowork ログインユーザーのメールアドレス・更新日時（JST）** を併せて記録する（単票モード等で行を新規に作る場合は 作成者・作成日時）。特定できなければ settings の `OPERATOR_EMAIL`、それも無ければ人へ確認。正本: `assets/schema/data-layout.yaml` の `operator_tracking`。列はヘッダ名で解決する。
- **「請求済」への遷移は本スキルは行わない** — 人が Gmail で送信したあと、コンソールのボタン
  または「Gmailを確認」の送信済み整合で更新する（v2 状態機械: 未請求→下書き作成済→請求済→入金済。
  差し戻しは人の操作のみ）。
