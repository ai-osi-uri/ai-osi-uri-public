...
- やる：Gmail検索 → 契約PDFを **取引先ID×方向**のフォルダ（`{契約書ルート}/02.取引先別/{取引先ID}.{企業名}/{受注（AR）｜発注（AP）}`）へ格納（重複スキップ）→ 契約マスタ起票（取引先ID・方向・状態）→ **状態=締結済**なら 受注=月次請求スケジュール／発注=月次支払スケジュール を各月展開 → 人レビュー。
