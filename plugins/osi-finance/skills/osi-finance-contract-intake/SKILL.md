...
- 取引先（正式名称・宛名）、請求先To/CC（営業管理表参照可）
  ...
  下の Step 4 を参照）
- 金額区分（**ラベルは settings §4-4 `BILLING_PATTERNS` の左辺から選ぶ**。既定は 月額固定／一括／都度。組織が
  チケット・回数券などを足していればそれも可）、金額（税込）、税率（既定10%）。
  課金パターンが `prepaid`（前受・消化型）のラベルなら **数量・単価(税込)** も読み取り、一括額(税込)=数量×単価 で置く
...
- 展開の仕方は **金額区分のラベルではなく課金パターン**（settings §4-4 `BILLING_PATTERNS`／台帳「発行者設定」項目=金額区分）で決める：
  | パターン | 受注(AR) → 月次請求スケジュール | 発注(AP) → 月次支払スケジュール |
  |---|---|---|
  | `monthly`（既定ラベル 月額固定） | 契約期間の各月に1行 | 各月に1行 |
