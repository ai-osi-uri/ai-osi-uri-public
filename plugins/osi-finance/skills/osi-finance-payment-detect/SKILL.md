...
**3 つ目の経路として、意味でも拾う（`osi-core:jev-judge`）。** 前回実行以降の「添付ありのメール」全件の件名と本文を
まとめて `jev_judge` にかけ、noul「このメールは、当社が支払うべき請求（請求書・利用料・顧問料などの支払依頼）を送ってきたものですか？」
を聞く。`yes ≥ 0.8` はキーワード・送信元で拾えていなくても請求書候補に加え、`unsure` は要確認に回す。
金額・請求書番号・期日の読み取りと台帳との照合は従来どおり Claude が行う（Jev は数字の比較が苦手）。
