...
**「コンソールが無い」「HTML が出てこない」と言われたら**、次の順で切り分ける。
コンソールの画面はプラグイン（スキル）ではなく**拡張（.mcpb）の中**にあり、
入口の `コンソールを開く.html` は台帳フォルダに置かれる。

1. `console_url` が呼べるか。呼べない → 拡張が入っていない。ツールポータルから入れてもらう
2. 呼べて `started: false` → 返ってきた `hint` のとおり（台帳フォルダ未設定・console_http off など）
3. 呼べて URL が出るのに HTML が無い → `health_check` を呼ぶ。拡張が台帳フォルダに作り直す
   （v1.46.0 以降。それより古い拡張は、台帳フォルダを設定したあと Claude Desktop を開き直すと作られる）
4. 拡張の無い PC 向けにも、台帳と一緒に `assets/templates/コンソールを開く.html` を置いておける。
   開くと「何が足りないか」を画面で案内する
