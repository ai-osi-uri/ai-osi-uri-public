# 導入完了レポート — テンプレート

`osi-finance-setup` がステップ7で**自動生成**する「導入完了レポート」の雛形。
QA-リリースチェックリスト.md「B. 導入直後スモークテスト」を1枚に転記し、各項目を
**OK / 要対応 / スキップ** で埋める。**機微値（口座番号・登録番号・実在支払先名）は書かない。**
判定は読み取り・ドライランのみで行い、**書き込み・送金・自動確定はしない。**

---

## OSI Finance 導入完了レポート

- 組織：{組織名（非機微の表示名）}
- 実施日：{YYYY-MM-DD}
- 実施者：{担当}
- plugin：osi-finance v{バージョン}

### B表スモーク結果

| # | 項目 | 結果 | メモ（件数・要対応の内容。機微値は書かない） |
|---|---|---|---|
| 1 | `osi-finance-settings.md` 生成（社名・税率・採番・Driveルート） | {OK/要対応} | {例: 支払先マッピングは後追い} |
| 2 | Drive フォルダ作成（契約書／受領請求書／送付請求書／カードSaaS証憑） | {OK/要対応} | {作成 or 既存再利用} |
| 3 | 台帳テンプレ配置（共通マスタ／請求管理台帳／支払管理台帳／仕訳台帳）＋「発行者設定」記入 | {OK/要対応/スキップ} | {既存ありで上書き回避なら スキップ。発行者設定の必須項目が未記入なら 要対応（値は書かない）} |
| 4 | 請求(AR)：osi-finance-invoice 当月ドライラン | {OK/要対応} | {当月の未請求 N 件／合計 ¥X（0件も可）} |
| 5 | 受領検出(AP)：osi-finance-payment-detect | {OK/要対応} | {新着 N 件／新着なし} |
| 6 | 突合(AP)：osi-finance-mf-sync / freee-sync 計上漏れ0確認（ACCOUNTING_SYNC=none はスキップ） | {OK/要対応/スキップ} | {計上漏れ N 件} |
| 7 | ダッシュボード：osi-finance-dashboard 描画・AR数値 | {OK/要対応} | {費用構成・純損益・AR が出たか} |
| 8 | コネクタ疎通（MF※mf のとき／Drive＋ローカル同期／メール） | {OK/要対応} | {未接続: …} |
| 9 | 日次/月次タスク「Run now」で許可先取り | {OK/要対応/未実施} | {登録/案内した周期} |

### サマリ

- OK：{n} 件 ／ 要対応：{m} 件 ／ スキップ：{k} 件
- 導入判定：**{完了 / 一部要対応}**

### 要対応リスト（あれば、優先順）

1. {何を・なぜ・次アクション（例: Superhuman 接続、テンプレ再配置、支払先マッピング追記）}
2. …

### 次の一手（日常運用への引き継ぎ）

- 「契約を取り込んで」→ osi-finance-contract-intake／「今月の請求書を作って」→ osi-finance-invoice
- 「この請求書払って」→ osi-finance-payment-intake／「今月の経理を締めて」→ osi-finance-monthly
- 詰まったら `docs/エラー処理ガイド.md` を参照（大容量PDF＝ローカル同期／添付＝Superhuman）。
