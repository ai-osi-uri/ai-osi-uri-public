...
6. **台帳ファイル名**：請求管理台帳（既定 `請求管理台帳`）／支払管理台帳（既定 `支払管理台帳`）。
   **運用形式はネイティブ Google スプレッドシート**（xlsx テンプレは搬送用。ステップ3で変換配置）
...
├── 00.契約書/
├── 01.受領請求書/           （支払管理台帳はここ）
├── 02.送付請求書/           （請求管理台帳はここ。発行PDFは YYYY-MM/ 配下）
└── 03.経費管理/
    ├── カードSaaS証憑/
...
## ステップ 3: 台帳の配置（Google スプレッドシート変換）

**運用正本はネイティブ Google スプレッドシート**（同時編集・API 読み書き・ローカル同期の
不安定さ回避のため）。同梱の xlsx テンプレは搬送用で、配置時に変換する。

1. `assets/templates/請求管理台帳_テンプレート.xlsx` を `02.送付請求書/`（settings の実値）に
   アップロードし、**Google スプレッドシート形式に変換**して台帳ファイル名で保存する。
2. `assets/templates/支払管理台帳_テンプレート.xlsx` を `01.受領請求書/` に同様に変換配置する。
3. 変換後の**スプレッドシートIDを settings の `LEDGER_BILLING_SHEET_ID` / `LEDGER_PAYMENT_SHEET_ID` に記録**する
   （以後の読み書き・自動追記はこの ID を使う）。
4. 変換アップロードが使えない環境では、フォールバックとして xlsx を Drive Web UI から
   「Google スプレッドシートとして保存」する手順を案内する（ローカル同期フォルダ経由の xlsx 配置は
   証憑 PDF 等の大容量ファイル向けの手段として残す）。
...
2. **台帳ヘルスチェック**：`assets/schema/data-layout.yaml` と照合し、①ルート配下に required な
   フォルダが揃っているか、②各台帳に required なタブが揃っているか、③各タブの1行目ヘッダが
   仕様の列と一致するか、を検査する。**差分があっても自動修復しない**——欠け・不一致を一覧で報告し、
...
- **台帳テンプレの変換配置が失敗**：Drive Web UI で xlsx を開いて「Google スプレッドシートとして保存」するフォールバックを案内し、保存後にスプレッドシートIDを settings に記録する。既存台帳は**上書きしない**。（証憑 PDF など大容量ファイルの格納は従来どおり**ローカル同期フォルダ経由**を推奨。）
