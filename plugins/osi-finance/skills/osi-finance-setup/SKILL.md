...
6. **台帳ファイル名**：請求管理台帳（既定 `請求管理台帳`）／支払管理台帳（既定 `支払管理台帳`）。
   **保存形式**（`LEDGER_BACKEND`）を選ぶ：`xlsx`＝ローカルの Excel ファイル（Drive 等の同期
   フォルダに置ける・既定）／`gsheet`＝ネイティブ Google スプレッドシート。ステップ3で配置する
...
保存形式は2通り。**どちらでも全スキルが同じツール名で動く**（拡張が吸収する）。
ステップ2で選んだ `LEDGER_BACKEND` に従って配置する。

### A. xlsx（既定・ローカルの Excel ファイル）
...
### B. gsheet（ネイティブ Google スプレッドシート）

同時編集が必要な場合はこちら。

1. `assets/templates/*.xlsx` を Drive にアップロードし、**Google スプレッドシート形式に変換**して
   台帳ファイル名で保存する。
2. 変換後の**スプレッドシートIDを settings の `LEDGER_BILLING_SHEET_ID` / `LEDGER_PAYMENT_SHEET_ID` に記録**する。
3. 変換アップロードが使えない環境では、Drive Web UI で xlsx を開き「Google スプレッドシートとして
   保存」する。
...
- **台帳テンプレの配置が失敗**（gsheet の場合）：Drive Web UI で xlsx を開いて「Google スプレッドシートとして保存」するフォールバックを案内し、保存後にスプレッドシートIDを settings に記録する。既存台帳は**上書きしない**。（証憑 PDF など大容量ファイルの格納は従来どおり**ローカル同期フォルダ経由**を推奨。）
