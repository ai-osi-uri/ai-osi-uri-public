---
name: osi-finance-payment-intake
description: >
  受領請求書（AP）を「①受領・格納 → ②読取・科目/税区分判定 → ③支払管理台帳に支払予定を起票
  → ④振込情報の整形提示」まで進めるオンデマンド・スキル。台帳へ書くタイミングは設定の
  `WRITE_CONFIRMATION`（事前確認=人が確認してから反映／事後確認=先に反映してから報告）に従う。**振込（振込口座から実行・例: Trunk）で
  払う受領請求書のみ対象**で、カード払い（自動課金SaaS含む）は会計側のカード連携が自動仕訳するため
  台帳に起票しない（証憑保存のみ）。入口は3系統：**Cowork に直接アップロード（PDFをドロップ）**、Drive 手置き（受領請求書フォルダ）、メール添付。
  サミダレで届く請求書を、その場で Cowork にドロップ→格納→起票、まで回せる。
  **送金しない**（人が振込口座で振込実行）。
  「この請求書払って」「○○社の請求書を支払予定に」「受領請求書を台帳に起票」「振込で払う請求書を
  起票」「受け取った請求書を支払管理台帳に入れて」「請求書PDFを読み取って支払予定にして」などで発動。
  月次クローズ・突合は osi-finance-monthly / osi-finance-mf-sync、請求書発行（AR）は osi-finance-invoice、
  契約取込は osi-finance-contract-intake の役割（本スキルは行わない）。日次の取りこぼし検出は osi-finance-payment-detect。
requires_connectors:
  - server: superhuman
    provision: user-install
  - server: money-forward
    provision: user-install

---

# osi-finance-payment-intake（受領請求書 → 支払予定 起票）

> **組織固有値はプロファイルから読む。** 本文の `{{paths.*}}` `{{ledgers.*}}` `{{company.*}}` `{{members.*}}` は、連結フォルダ直下の `osi-profile.md`（雛形: osi-core の `plugins/osi-core/skills/getting-started/assets/osi-profile.example.md`）の値に置き換えて解釈する。無ければ会社名・案件フォルダ・台帳の有無・使うコネクタを質問して先に作る。値をここに直書きしない。

> **組織固有値（社名・登録番号・振込先・税率・採番ルール・Drive ルート／フォルダ名・台帳ファイル名・
> 支払先→科目マッピング等）は `{{paths.finance}}/osi-finance-settings.md`（テンプレ：`config/osi-finance-settings.example.md`）
> を参照する。** 実値版が無ければユーザーに作成を案内する。

サミダレで届く受領請求書（AP）を、人がレビューして振込口座（例: Trunk）で振り込む一歩手前まで整える。
**会計の二重計上を構造的に防ぐ**ため、振込で払うものだけを台帳に起票し、カード払いは起票しない。

## 確定した設計（厳守）

- **振込（振込口座から実行・例: Trunk）で払う受領請求書のみ対象。** カード払い（自動課金SaaS含む）は対象外＝台帳に起票しない。
  カードは会計側のカード連携（法人カード・例: UPSIDER）が自動仕訳するため、台帳に入れると **会計SaaSで二重計上**になる。
  カード払いの証憑は `osi-finance-settings` の `カードSaaS証憑` フォルダに**保存のみ**。
- **支払先マスタの正本＝支払管理台帳。** 照合は台帳基準（MFの getTradePartners は補助）。
  台帳の支払先マスタに「既定の支払方法（振込／カード）」を持たせ、それで振込／カードを振り分ける。
  新規・不明な相手はその場で確認する。
- 入口は3系統（どれでも同じ処理に合流する）：
  - (a) **Cowork 直接アップロード（推奨・サミダレ運用の主役）** … ユーザーが請求書PDFを Cowork にドロップして
    「この請求書を支払予定に起票して」と言う。アップロードされた PDF をその場で読み、Drive の受領請求書フォルダに格納してから処理する。
  - (b) **Drive 手置き** … `osi-finance-settings` の `受領請求書` フォルダに置かれた PDF。
  - (c) **メール添付** … 拡張の `mail_save_attachments` で取引先からの請求書PDF添付を受領請求書フォルダへ直接保存する（完了メール等に添付）。メールの特定は純正のメールコネクタの検索で行い、その ID をそのまま渡す。

## 役割と非役割

- やる：受領・格納 → 読取 → 支払先マッチング → 科目・税区分判定 → 台帳に支払予定起票（`WRITE_CONFIRMATION` に従う）→ 振込情報の整形提示。
- やらない：**送金実行**（人が振込口座（例: Trunk）で実施）、会計仕訳の自動投入（= osi-finance-mf-sync / osi-finance-monthly が担う）、
  AR 請求書の発行（= osi-finance-invoice）、契約取込・請求スケジュール展開（= osi-finance-contract-intake）。
- **台帳へ書くタイミングは設定 `WRITE_CONFIRMATION` に従う**（§5）。`事後確認` の組織で「案を出して人の返事を待つ」と、
  返事が来ないまま会話が終わり、**請求書は Drive にあるのに台帳に無い**状態になる（2026-09-05 に実際に起きた）。
  送金・会計SaaSへの計上確定は、どちらのモードでも行わない。

## 前提コネクタ

- Google Drive / 台帳（受領請求書の格納・支払管理台帳）。
- AI OSI URI Finance 拡張の `mail_list_attachments` / `mail_save_attachments`（v1.44.0〜。Gmail／Outlook 両対応）で請求書PDF添付を取得。予備として Superhuman Mail の `get_attachment`。
- MF会計（任意）：`getTradePartners` を支払先マッチングの補助に使う（正本は台帳）。

## 手順

### 1. 受領・格納
入口に応じて PDF を取得し、`{請求管理ルート}/{受領請求書}/YYYY-MM/`（既定: `{{paths.finance}}/01.受領請求書/YYYY-MM/`）に格納する。
- (a) **Cowork 直接アップロード**：ユーザーがドロップした PDF をそのまま読み、上記フォルダにコピーして格納する
  （bash で Drive のローカル同期パスへ `cp`。クラウドのみで `cp` が失敗する場合は Read で一度ダウンロードしてから格納）。
- (b) Drive 手置き：当該フォルダの新規 PDF を対象にする。
- (c) メール添付：`mail_list_attachments` で添付を確かめ、`mail_save_attachments(message_id, dest_dir=受領請求書/YYYY-MM, only_pdf=true)` で格納する。保存名は元のファイル名になるので、そのあと下の命名規則に合わせて名前を変える。同名があれば拡張が `_2` を付けて上書きしない。
- 命名：`支払先_請求書番号_受領日.pdf`。**同名・同一請求書（支払先＋請求書番号）が既にあればスキップ**（重複防止）。

### 2. 読取
請求書PDFから以下を抽出する。**読めない項目は推測せず「要確認」**にする。
- 支払先の正式名称、請求書番号、請求日、支払期日
- 金額（税抜／消費税／税込）
- インボイス登録番号（T＋13桁）、源泉の有無

### 3. 支払先マッチング
台帳の支払先マスタに照合し、支払先ID（例: `V-013`、形式は `osi-finance-settings` の採番ルール）を特定する。
- 新規の支払先は採番ルールで次番を採る。`事前確認` なら候補として提示、`事後確認` ならそのまま採番して
  取引先マスタ・支払先マスタに書き、報告で「新規支払先として追加した」と明示する。
- **既定の支払方法がカード**なら → 「**カード払い → 台帳起票せず、証憑を `カードSaaS証憑` に保存のみ**」で終了。
  （理由：会計側のカード連携が自動仕訳するため。台帳に入れると二重計上になる。）

### 4. 科目・税区分判定
`osi-finance-monthly/references/monthly-rules.md`（判断辞書）と `osi-finance-settings` の支払先→科目マッピングに従い、
**勘定科目・税区分(MF)・インボイス区分・源泉**を判定する。迷うものは「要確認」とし確定しない。
- **1枚に複数科目が混在する請求書**（例：社宅契約の初期費用＝賃料＋敷金＋礼金＋保証料）は、**明細ごとに行を分けて起票**する
  （地代家賃／差入保証金／長期前払費用 等）。不動産・社宅・非課税まわりは monthly-rules の「不動産/社宅系」を参照（敷金=差入保証金、
  礼金20万円以上=長期前払費用、居住用賃料=非課税仕入 等）。資産/費用の別・償却年数は**税理士確認**。

### 5. 台帳起票（設定 `WRITE_CONFIRMATION` に従う）
支払管理台帳「月次支払管理」に1行を作る。**いつ人が見るか**は `osi-finance-settings.md` の「起票確認方式」で決まる：

- **事前確認（既定）**：金額・科目・税区分・期日を一覧で提示し、人が確認・修正してから `sheets_append_row` で反映する。
- **事後確認**：読み取れた内容で**先に `sheets_append_row` で書いてから**、書いた行（支払先・請求書番号・金額・期日・受領請求書ファイル）と
  「要確認」にした項目を報告する。人はコンソールの支払一覧で見て直す。読めない値は推測せず、備考に「要確認」と理由を残して書き込みは続行する。
  **「案です、よければ書きます」で止めない。** 止めると返事が来ないまま終わり、台帳に載らない。

設定が無い/読めない場合は事前確認（安全側）で動く。
- `支払ステータス`＝`未払い`、`支払方法`＝`振込`。`突合結果`・`突合日時` は空のまま（会計・銀行との突合で mf-sync / freee-sync / bank-recon が書く）。
- **請求書ID（`支払先ID + 対象月(YYYYMM) + 請求書番号`）は列ではない。** 仕訳の摘要に入れる突合キーで、台帳には
  `支払先ID`・`対象月`・`請求書番号` を別々に書く。`MF連携状態` という列も無い（旧スキーマの名残。突合状況は `突合結果` で見る）。
  存在しない列を渡すと書き込みはエラーで止まる。
- **`受領請求書ファイル` 列に、§2 で格納した PDF のパスを必ず書く（台帳ルートからの相対パス）。**
  例: `01.受領請求書/202607/サンプル株式会社_F-1-20260706-000001-01_20260709.pdf`
  ここが空だと、あとから「どの行がどの請求書か」を人が突き合わせ直すことになる。
  2026-08-17 に過去45行を遡って突き合わせたが、**ファイル名から相手を特定できず7行しか埋まらなかった。**
  起票の瞬間なら確実に分かるので、その場で書く。コンソールの支払一覧から開けるようになる。
- 新規支払先は「支払先マスタ」にも1行（支払先ID採番＋振込先口座＋**`取引先ID`**）を加える。
  **支払先マスタは会社の名前を持たない**（2026-08-18 に支払先名・主担当者名を削除。法人/個人の `支払先区分` は源泉の判定に使うので残す）。
  名前・正式名称は**共通マスタの「取引先マスタ」**が正本で、`取引先ID`（`P-001` 形式）で引く。
  取引先マスタに無い相手なら、**先に取引先マスタへ1行足してから**支払先マスタを作る。
- 金額・科目・税区分・期日の提示と台帳反映の順番は、上の `WRITE_CONFIRMATION` のとおり。
- 列は `assets/schema/data-layout.yaml` の「月次支払管理」が正本（下記§台帳列）。

**台帳への書き込み方法**

`sheets_append_row`（**AI OSI URI Finance 拡張**。同名ツールが Deploy 拡張にもあるので取り違えないこと）で追記する。

1. `事前確認` なら起票案を人に提示して OK をもらう。`事後確認` ならそのまま次へ。
2. `sheets_append_row` に `tab`（「支払先マスタ」＝新規時 / 「月次支払管理」）と **`row`＝列名→値**を渡す。列順に依存しないので、台帳の列が増減しても壊れない。
3. **`dedupe_by` に `["請求書番号","支払先ID"]` のような一意キーを指定する。** 同じ行が既にあれば追記されず、既存行の位置が返る（二重起票の防止）。
4. 台帳に無い列名を渡すとエラーになる（黙って捨てられないので、列名の取り違えはその場で分かる）。書き込み後の検算は拡張側が読み戻しで行う。

不安なら `dry_run: true` で入る位置と値を確認してから実行する。

**拡張の道具が無ければ、同梱の `assets/scripts/ledger_io.py` で同じことをする**（引数は `sheets_append_row` にそろえてある。
書く前に `_backup/` へ控え、`受領請求書ファイル` が実在しないと止まる）：

```bash
L="python3 assets/scripts/ledger_io.py <経理フォルダ>"
$L next-id --tab 支払先マスタ --col 支払先ID --prefix V-      # 新規の支払先だけ（取引先マスタ→支払先マスタの順）
$L append --tab 月次支払管理 --dedupe-by 請求書番号 支払先ID --row '{"支払先ID":"V-001","支払先名":"サンプル株式会社",
  "支払カテゴリ":"外注費","対象月":"2026/08","支払金額（税抜）":50000,"消費税額":5000,"支払金額（税込）":55000,
  "請求書番号":"F-0917","請求書受領日":"2026-09-05","支払予定日":"2026-09-30","支払ステータス":"未払い","支払方法":"振込",
  "受領請求書ファイル":"01.受領請求書/2026-09/サンプル株式会社_F-0917_20260905.pdf"}'
```

### 6. 支払依頼の整形（送金しない）
振込先・**振込額 ＝ 税込 − 源泉徴収税額**・支払期日 を整形して提示する。
**送金は実行しない**（人が振込口座（例: Trunk）で振込を行う）。

### 7. 支払後更新（任意・頼まれたら）
支払が済んだら、状態＝`支払済`・支払日 を記入する。
以後は `osi-finance-mf-sync` が MF と突合し、`osi-finance-monthly` が月次クローズで処理する。

## 台帳列（「月次支払管理」／ledger-schema 準拠）

支払先ID／支払先名／支払カテゴリ／対象月／契約タイプ／月額固定（税抜）／稼働時間／時間単価額（税抜）／
実費・その他（税抜）／支払金額（税抜）／消費税額／源泉徴収税額／支払金額（税込）／請求書番号／請求書受領日／
支払予定日／支払日／支払額／差額／支払ステータス／遅延日数／承認者／備考／作成者／作成日時／更新者／更新日時／
支払方法／突合結果／突合日時／**受領請求書ファイル**

> 列名は実台帳（支払管理台帳「月次支払管理」）と `data-layout.yaml`（layout_version 8）に合わせてある。以前ここに書いてあった一覧は
> 旧スキーマのままで実物と食い違っていた（`請求書ファイル` という列は存在しなかった）。
> **`受領請求書ファイル` は 2026-08-17 に実台帳へ追加した列。**yaml とテンプレートは 2026-09-24 まで追従しておらず、
> 空環境ではこの列（と 突合結果／突合日時）に書けなかった。スキルが書く列は必ず yaml にあること（`build_templates.py --check`）。

## 境界（重複防止のための線引き）

- **振込のみ起票**。カードは起票せず証憑保存のみ（会計のカード連携が自動仕訳）。
- 月次クローズ・台帳⇔MFの突合は **osi-finance-monthly / osi-finance-mf-sync**（本スキルは起票まで）。
- 自社の請求書発行（AR）・入金は **osi-finance-invoice**（本スキルは受領＝AP 専用）。
- 契約取込・請求スケジュール展開は **osi-finance-contract-intake**。
- 日次の受領請求書 取りこぼし検出は **osi-finance-payment-detect**（本スキルはオンデマンド起票）。

## 留意

- 送金しない。台帳へ書くタイミングは `WRITE_CONFIRMATION`（事後確認なら書いてから報告）。
- 読めない値は推測せず「要確認」。新規・不明な支払先はその場で確認。
- 機微値（実口座・登録番号・支払先名）はスキルに直書きせず `osi-finance-settings` から読む。

## エラー処理

詳細は **[`docs/エラー処理ガイド.md`](../../docs/エラー処理ガイド.md)** を正本とする。本スキルで詰まりやすい点：

- **メール添付（受領請求書PDF）が取得できない**：純正のメールコネクタでは不可。拡張の `mail_save_attachments` を使う（未接続なら「メールにつないで」）。拡張が使えない環境だけ Superhuman の `get_attachment`、それも無ければ **Drive手置き**（受領請求書フォルダ）を案内して止める。
- **大容量の請求書PDF**は Drive の**ローカル同期フォルダにコピー**して格納（直アップロードは不安定）。
- 読めない値は**推測せず「要確認」**。新規・不明な支払先はその場で確認。**送金しない**。台帳へ書くタイミングは `WRITE_CONFIRMATION` に従う。
