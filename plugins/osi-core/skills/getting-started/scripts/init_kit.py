...
- 作業を始める前に、これから何をするかを **抽象的に 1 行**で示す（例：「① 資料を読む → ② 骨子を作る → ③ スライドにする。いまは①」）。細かい手順は書かない。
- 返事の最後は **「次にやること：〜」を 1 行**。相手が押すもの・送るものが 1 つだけ分かるように書く。
- あわせて **「ほかにできること」を選択肢で 2〜4 個**出す（選択肢を出す機能があればそれを使う）。選択肢は「〜を作る」「〜を調べる」のように、選ぶと何が起きるかで書く。
- 専門用語は使わない。使うときは一言で言い換える。
""")
