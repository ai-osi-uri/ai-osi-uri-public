...
| plugins/osi-finance/docs/導入手順書.md | 111 | 共有ドライブ | >   共有ドライブなら `G:\共有ドライブ\AI OSI URI\06.会計` のような形になります |
...
| plugins/osi-finance/skills/osi-finance-invoice/SKILL.md | 74 | CAIO | - **グループが複数の対象月にまたがる場合は、対象月を摘要・明細名に必ず残す**（例「CAIO業務 2026年4〜6月分」）。 |
| plugins/osi-finance/skills/osi-finance-invoice/SKILL.md | 103 | 共有ドライブ | - **マウント済みの Drive 共有ドライブ（ローカル同期フォルダ＝FS）** に base64 をデコードして書き出す： |
